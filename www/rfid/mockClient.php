...
Class MockClient {
	...
	<form action='/rfid/signup/' method='post'>
		<input name='pass' type=text value='a94a8fe5ccb19ba61c4c0873d391e987982fbbd3' />
...
END;
	...
	<form action='/rfid/auth/' method='post'>
		<input name='login' type=text value='test' />
		<input name='pass' type=text value='test' />
...
END;
	...
	<form action='/rfid/post/' method='post' name='form1'>
		...
		<br><input onclick='form1.checksum.value = SHA1(form1.json.value)' value='Пересчитать' type=button>
		<br><input onclick='form1.checksum.value = SHA1(form1.json.value)' type=submit>
		<input name='checksum' size=44 type=text value="72ba4e775c77af052353e5022484dac60aa2150d">
	</form>
END;
	}
}

$app->get('/signup/', 'MockClient::ViewSignup');
$app->get('/auth/', 'MockClient::ViewAuth');
